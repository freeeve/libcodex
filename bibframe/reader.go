...
const (
	...
	pNonSortNum            = bflcNS + "nonSortNum"
	pVariantType           = bfNS + "variantType"
	classVariantTitle      = bfNS + "VariantTitle"
	classParallelTitle     = bfNS + "ParallelTitle"
	...
	pNote                  = bfNS + "note"
	pNoteType              = bfNS + "noteType"
	pTableOfContents       = bfNS + "tableOfContents"
	classNote              = bfNS + "Note"
...
// The bf:associatedResource target of a 76x-78x bf:relation is likewise a nested
// related Work (reconstructed as a linking-entry field), so it is skipped too.
func relatedWorkSet(g *rdf.Graph) map[rdf.Term]bool {
	m := map[rdf.Term]bool{}
	for _, t := range g.Triples {
		if t.P.Kind == rdf.IRI && (t.P.Value == pRelatedTo || t.P.Value == pAssociatedResource) {
			m[t.O] = true
		}
	}
	return m
